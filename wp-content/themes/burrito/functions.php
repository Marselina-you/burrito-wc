<?php
/**
 * burrito functions and definitions
 *
...
function burrito_setup() {
	/*
		...
		*/
	load_theme_textdomain( 'burrito', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'burrito' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'burrito_custom_background_args',
			array(
...
function burrito_scripts() {
	wp_enqueue_style( 'burrito-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'burrito-style', 'rtl', 'replace' );

	wp_enqueue_script( 'burrito-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
